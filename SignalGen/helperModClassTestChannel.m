...
%helperModClassTestChannel Test channel for signal propagation
  ...
  methods(Access = protected)
    function setupImpl(obj)
      obj.MultipathChannel = comm.RicianChannel(...
        ...
        'KFactor', obj.KFactor, ...
        'DirectPathDopplerShift',obj.DirectPathDopplerShift,...
...
%       yInt1 = yInt1.*exp(1j*2*pi*obj.DopplerShift*t');
    ...
    function [out,noise, H] = addNoise(obj,in, H)
      ...
      noise = sqrt(obj.ALPHA/2)*(randn((size(in,1)),1) + 1j*randn(size(in,1),1));
      npow = rms(noise)^2;
      sigpow = rms(in)^2;
      ratio = sqrt(npow*10^(obj.SNR/10)/sigpow);
      sig = ratio*in;
      H = ratio*H;
      out = sig + noise;
%       out = awgn(in,obj.SNR);
