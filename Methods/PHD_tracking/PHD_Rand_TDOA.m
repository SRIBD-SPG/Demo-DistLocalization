function [track_X_lla_tdoa] = PHD_Rand_TDOA(tdoa_signal,z_truth,senPos)
...
%% 基本参数（可修改）
monteCarlo = 1; %蒙特卡洛次数
NumSen = size(senPos, 2); %传感器数目
Ttotal = size(tdoa_signal, 2); T = 0.1; %总时间和采样间隔
x_dim = 6; %运动状态维度
c_dim = 1; %类信息维度
z_dim = 1; %观测维度
up_num = NumSen - 1; %迭代更新次数
...
for mc = 1 : monteCarlo
    ...
    B_bir(:, :, 1) = diag([ 30; 20; 30; 20; 1e-8; 1e-8]/1)*1e0;%高sigma
    ...
    for t = 1 : Ttotal
       ...
       for s = 1 : up_num
           if ~isempty(tdoa_signal{1, t}(s, 1))
               idx = find(rand(1, size(tdoa_signal{1, t}(s, 1), 2)) <= 1); 
               Z{t, mc}{s, 1}(1, :) = tdoa_signal{1, t}(s, idx) * v;
               Z{t, mc}{s, 1}(2, :) = 1;
           else
               Z{t, mc} = []; 
           end
        ...
        for s = 1 : size(Z{t, mc}, 1)
            ...
            if ~isempty(Z{t, mc}{s, 1})
                ...
                for i = 1 : size(m_up, 2)
                    ...
                    d1 = sqrt((m_up(1, i) - senPos(1, 1)) ^ 2 + (m_up(3, i) - senPos(2, 1)) ^ 2 + (m_up(5, i) - senPos(3, 1)) ^ 2); %目标到传感器1的距离
                    ds = sqrt((m_up(1, i) - senPos(1, s + 1)) ^ 2 + (m_up(3, i) - senPos(2, s + 1)) ^ 2 + (m_up(5, i) - senPos(3, s + 1)) ^ 2); %目标到传感器s+1的距离
                    H(1, 1, i) = (m_up(1, i) - senPos(1, s + 1)) / ds - (m_up(1, i) - senPos(1, 1)) / d1; 
                    H(1, 3, i) = (m_up(3, i) - senPos(2, s + 1)) / ds - (m_up(3, i) - senPos(2, 1)) / d1; 
                    H(1, 5, i) = (m_up(5, i) - senPos(3, s + 1)) / ds - (m_up(5, i) - senPos(3, 1)) / d1;
                    ...
                    for j = 1 : size(Z{t, mc}{s, 1}, 2)
                        M_dis(i, j) = (Z{t, mc}{s, 1}(1, j) - Z_pre(1, i))' * (Z{t, mc}{s, 1}(1, j) - Z_pre(1, i)); 
                        like(i, j) = 1 / sqrt(det(2 * pi * S(:, :, i))) * 1 / ( M_dis(i, j) ); 
                        is_corr = m_up(end, i) ~= Z{t, mc}{s, 1}(2, j); 
                        is_corr = is_corr + 1; 
                        w_up2(i, j) = w_up(i) * (P_D / sqrt(det(2 * pi * S(:, :, i))) * ...
                                      exp(-0.5 * (Z{t, mc}{s, 1}(1, j) - Z_pre(1, i))' * pinv(S(:, :, i)) * (Z{t, mc}{s, 1}(1, j) - Z_pre(1, i))) + 1e-299)...
            ...
            for i = 1 : size(m_up, 2)
                %如果该目标和某个量测之间的距离小于门限，则用该量测更新目标的信息
                 if uasses(i) ~= 0
                    ...
                    m_up(1 : end - 1, i) = m_up(1 : end - 1, i) + K(:, :, i) * (Z{t, mc}{s, 1}(1, uasses(i)) - Z_pre(1, i)); 
...
track_X_lla_tdoa = track_lla(1:2,:);
end